	...
	if($_GET['add']=="inf") {
		...
		if( !empty($name) && !empty($description) && !empty($starting_datetime) && !empty($ending_datetime) && !empty($price) ) {
			...
			if($json_data['code'] !== 200){
				...
				@header("Location: dashboard.php?p=deals&action=error");
				echo "<script>window.location='dashboard.php?p=deals&action=error'</script>";

			} else {
				//echo "<div class='alert alert-success'>".$json_data['msg']."</div>";
				@header("Location: dashboard.php?p=deals&action=success");
				echo "<script>window.location='dashboard.php?p=deals&action=success'</script>";
			}
...
<div id="adddeals" class="popup">
	<div class="popup_container col40">
		...
		<div class="paddingallsides form">
			...
			<form action="dashboard.php?p=deals&add=inf" id="hoteldealsfrm" method="post" enctype="multipart/form-data"> 
				...
				<div class="left col30" style="margin-right: 10px;">
					...
					<div class="itm"> 
						...
						<div align="right" style="margin-top:10px;">
							...
							| 
							<a href="dashboard.php?p=deals&del=ok&id=<?php echo $vaaal['Deal']['id']; ?>" onclick="return confirm('Do you really want to delete hotel deal?');" class="edit_icn" >
								Delete
							</a>
						</div>
